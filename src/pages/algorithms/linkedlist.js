...
import ReactFlow, { Background, Controls, MiniMap, useNodesState, useEdgesState, Handle, Position, MarkerType } from "reactflow";
...
class LinkedList {
  ...
  async insert(value, index, callback) {
    ...
    if (index === 0) {
      ...
      await callback({ step: 2, index: index, newNode, nextNode: current.next });
      await new Promise((resolve) => setTimeout(resolve, 500));

      newNode.next = current.next;
      await callback({ step: 3, index: index, newNode, nextNode: current.next });
      await new Promise((resolve) => setTimeout(resolve, 500));
  ...
  async remove(index, callback) {
    ...
    if (index === 0) {
      ...
      await callback({ step: 1, index, prevNode: prev, nextNode: current.next });
      if (index === this.length - 1) {
        this.tail = prev;
      }
      await new Promise((resolve) => setTimeout(resolve, 500));
      await callback({ step: 2, index, prevNode: prev, nextNode: current.next });
    }
...
const CustomNode = ({ data }) => (
  <div
    className={`w-6 h-6 rounded-full flex items-center justify-center text-white ${data.highlight ? "ring-2 ring-yellow-400" : ""
      }`}
...
const getEdgeStyle = (type = "default") => ({
  ...
  style: type === "insertion" ? insertionEdgeStyle : type === "removal" ? removalEdgeStyle : defaultEdgeStyle,
  animated: type !== "default",
  markerEnd: {
    type: MarkerType.ArrowClosed,
    width: 6,
    height: 6,
    color: type === "insertion" ? "#22c55e" : type === "removal" ? "#ef4444" : "#b1b1b7",
  },
});
...
const LinkedListVisualization = () => {
  ...
  const handleInsertionStep = useCallback(
        ...
        case 3:
          setEdges((eds) => {
            ...
            const updatedEdges = eds.filter(
              (edge) => edge.source !== (index - 1).toString() && edge.target !== index.toString()
            );
            // Add the new edges
            return [
              ...updatedEdges,
    ...
    [nodes, setNodes, setEdges, updateVisualization]
  );
    ...
    async ({ step, index, prevNode, nextNode }) => {
      switch (step) {
        ...
        case 2:
          ...
          setEdges((eds) => {
            const updatedEdges = eds.filter((edge) => edge.target !== removingNodeId);
            if (prevNode && nextNode) {
              updatedEdges.push({
  ...
  return (
    <Layout>
      ...
      <div className="w-full flex">
        <div className="w-3/4 h-full" ref={containerRef} style={{ height: containerHeight }}>
          <ReactFlow
            nodes={nodes}
            edges={edges}
        ...
        <div className="w-1/4 h-full p-4 flex flex-col space-y-8 overflow-y-auto">
          <div className="flex flex-col space-y-2">
            ...
            <span className="text-gray-600 text-sm">
              {selectedNode ? t("insert_after_pos", { index: selectedNode }) : t("insert_at_end")}
            </span>
          </div>
          ...
          <div className="flex flex-col space-y-2">
            ...
            <span className="text-gray-600 text-sm">
              {removingNodeId !== null ? t("remove_node_at", { index: removingNodeId }) : t("click_node_to_remove")}
            </span>
          </div>
          ...
          <div className="flex flex-col space-y-2">
            ...
            <span className="text-gray-600 text-sm">  {searchResult !== null && (
              searchResult === -1
                ? t('node_not_found')
