...
我不会前端技术，不懂现代 web 开发，顶多只懂一点点 html 和 css，在 Claude3.5 和 GPT4 的助攻下，完成了一些有意思的小组件。最近也开始在日常开发中使用 GPT-5.2 和 GPT-5.3。我把它们集成在这个[展示站点](https://gallery.selfboot.cn)，欢迎来体验。
...
除了上面提到的模型，我现在日常也会使用下面这些：

- **GPT-5.2 / GPT-5.3**：用于更深度的推理、架构讨论和更复杂的实现任务。
- **Codex CLI**：终端优先的开发流程，适合直接在仓库里改代码、重构和执行命令驱动开发。
- **Claude Code CLI**：作为另一个 CLI 编程助手，用于对比实现方案和交叉验证结果。
- **Cursor**：IDE 场景下的 AI 工作流，用于交互式编辑、快速迭代和在编辑器内对话式改代码。
